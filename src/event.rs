...
pub fn run<S: 'static, E>(
    ctx_builder: ContextBuilder,
    f: impl FnOnce(&mut Context) -> S + 'static,
) -> !
where
    S: EventHandler<E>,
    E: std::fmt::Debug,
{
    ...
    let physical_size = dpi::PhysicalSize::<f64>::from((
        ctx_builder.conf.window_mode.width,
        ctx_builder.conf.window_mode.height,
    ));
    assert!(physical_size.width >= 1.0 && physical_size.height >= 1.0); // wgpu needs surfaces > 0
    let window_builder = winit::window::WindowBuilder::new()
        .with_title(ctx_builder.conf.window_setup.title.clone())
        .with_inner_size(physical_size)
        .with_resizable(ctx_builder.conf.window_mode.resizable)
        .with_visible(ctx_builder.conf.window_mode.visible)
        .with_transparent(ctx_builder.conf.window_mode.transparent);
    ...
    let mut window = Some(window_builder.build(&event_loop).unwrap(/* TODO */));

    let mut ctx: Option<Context> = None;
    let mut visible = false;
    let mut state = None;

    // Workaround variable until we update winit to 0.27.0 (all platforms will recieve Resumed)
    let initialized = std::cell::Cell::new(false);

    let mut ctx_builder = Some(ctx_builder);

    event_loop.run(move |mut event, _, control_flow| {
        let mut initialize = || {
            assert!(!initialized.get());
            if let Some(ctx) = &mut ctx {
                ctx.gfx.recreate_surface();
            } else {
                let mut new_ctx =
                    ctx_builder.take().unwrap().build(window.take().unwrap()).unwrap(/* TODO */);
                state = Some((f.take().unwrap())(&mut new_ctx));
        ...
        if cfg!(not(target_os = "android")) && !initialized.get() {
            initialize();
        }
        ...
        if initialized.get() {
            if let (Some(ctx), Some(state)) = (&mut ctx, &mut state) {
                ...
                match event {
                    Event::WindowEvent { event, .. } => match event {
                        ...
                        WindowEvent::CursorMoved { .. } => {
                            ...
                            let res = state
                                .mouse_motion_event(ctx, position.x, position.y, delta.x, delta.y);
                            if catch_error(
                                ctx,
                                res,
                                state,
